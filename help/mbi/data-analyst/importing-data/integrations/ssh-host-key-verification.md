...
| --- | --- |
| The host keys are not yet enrolled | **[!UICONTROL Save & Test]** scans the bastion host and port, enrolls the host keys, and stores them for this connection. |
| The host keys are already enrolled | **[!UICONTROL Save & Test]** skips enrollment. It does not overwrite, rotate, or delete existing enrolled keys, even if live bastion keys no longer match. |
| The enrolled keys are missing, empty, or invalid | **[!UICONTROL Save & Test]** does not repair invalid trust material by itself. An Admin must run **[!UICONTROL Refresh SSH Host Keys]** or contact Support if errors continue |
