...
Below are instructions specific to the legacy version of the `Cohort Report Builder`. If you are interested in using the new version, see [New Architecture](../../administrator/account-management/new-architecture.md) for more information about migrating to a [!DNL MBI] New Architecture account.
...
The `Cohort Report Builder` is currently optimized for grouping users by a common `cohort date`. You might be interested in grouping the users by a similar activity or attribute - if that is the case, we would love to help! We recommend checking out [this tutorial on qualitative cohorts](../dev-reports/create-qual-cohort-analysis.md) to get started.
